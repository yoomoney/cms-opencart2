...
class ControllerExtensionPaymentYandexMoney extends Controller
{
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            if ($this->validate($this->request)) {
                ...
                if (!empty($this->request->post['yandex_money_market_categories'])) {
                    $this->request->post['yandex_money_market_categories'] = explode(',', $this->request->post['yandex_money_market_categories']);
                }
        ...
        $data['action'] = $this->url->link($prefix . 'payment/' . self::MODULE_NAME, 'token=' . $this->session->data['token'], true);
        $data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=payment', true);
        $data['kassa_logs_link'] = $this->url->link($prefix . 'payment/' . self::MODULE_NAME . '/logs', 'token=' . $this->session->data['token'], true);
        ...
        $arLang = array(
            ...
            'tab_row_sign','tab_row_cause','tab_row_primary','ya_version','text_license','market','metrika','pokupki','active','active_on',
        ...
        $data['mod_off'] = sprintf($this->language->get('mod_off'), $this->url->link($prefix.'payment/install', 'token=' . $this->session->data['token'] . '&extension=yandex_money', true));
    ...
    private function getBreadCrumbs($add = null)
    {
        ...
        if (!empty($add)) {
            $result[] = array(
                ...
                'href' => $this->url->link($this->getPrefix() . 'payment/' . self::MODULE_NAME . '/' . $add['href'], $params, true),
            );
        }
        return $result;
    }
    ...
    public function goCurl($type, $post)
    {
        ...
        $this->response->redirect($this->url->link($this->getPrefix().'/payment/yandex_money', 'err='.$data->error_description.'&token=' . $this->session->data['token'], true));
    }
    ...
    public function sendmail()
    {
        ...
        $url = $urlHelper->link($this->getPrefix() . 'payment/yandex_money/simplepayment', 'order_id=' . $order_id, true);
    ...
    public function refund()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['kassa_refund_amount'])) {
            ...
            if (empty($error)) {
                if (!$this->refundPayment($payment, $orderInfo, $amount, $comment)) {
                    ...
                    $this->response->redirect(
                        $this->url->link($this->getPrefix() . 'payment/yandex_money/refund', 'token=' . $this->session->data['token'] . '&order_id=' . $orderId, true)
                    );
                }
            }
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link($this->getPrefix() . 'payment/yandex_money/refund', 'token=' . $this->session->data['token'] . '&order_id=' . $orderId, true)
        );
