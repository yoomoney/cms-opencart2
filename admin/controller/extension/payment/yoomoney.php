...
class ControllerExtensionPaymentYoomoney extends Controller
{
    ...
    private function validateWallet(Request $request)
    {
        ...
        $enabled = false;
        if (isset($request->post['yoomoney_wallet_enabled']) && $request->post['yoomoney_wallet_enabled'] === 'on') {
            $enabled = true;
        }
        ...
        $value = false;
        if (isset($request->post['yoomoney_wallet_create_order_before_redirect']) && $request->post['yoomoney_wallet_create_order_before_redirect'] === 'on') {
            $value = true;
        }
        ...
        $value = false;
        if (isset($request->post['yoomoney_wallet_clear_cart_before_redirect']) && $request->post['yoomoney_wallet_clear_cart_before_redirect'] === 'on') {
            $value = true;
        }
    ...
    {
        $data = array();
        $link = $this->url->link($this->getPrefix().'payment/yoomoney', 'token='.$this->session->data['token'],
            'SSL');
        ...
        if (isset($this->request->post['update']) && $this->request->post['update'] == '1') {
            ...
            $logs     = $this->url->link($this->getPrefix().'payment/yoomoney/logs',
                'token='.$this->session->data['token'], 'SSL');
            if (!empty($fileName)) {
